...
import { formatDuration, sendDailyPerformanceEmail, sendGmailMessage } from '../_lib/gmail'
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const body = readBody<{
      ...
      startTime?: string
      endTime?: string
      totalWorkingTimeSeconds?: number
    ...
    return json(res, err.status || 500, {
      error: err.message || 'Failed to send performance email',
